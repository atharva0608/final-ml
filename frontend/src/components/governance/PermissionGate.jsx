...
const PermissionGate = ({
  ...
  // Admin bypass — org_admin and super_admin should NEVER be gated
  const isAdmin = user && ADMIN_ROLES.includes(user.role);
  if (isAdmin) {
    return <>{children}</>;
  }

  return <PermissionGateInner
    featureId={featureId}
    resourceId={resourceId}
    sectionName={sectionName}
    sectionDescription={sectionDescription}
  >{children}</PermissionGateInner>;
};

/**
 * Inner component that actually runs the permission check.
 * Only rendered for non-admin users.
 */
const PermissionGateInner = ({
  featureId,
  resourceId = null,
  children,
  sectionName = "this section",
  ...
  return (
    <div className="relative min-h-screen">
      ...
      <div
        className={`transition-all duration-300 ${!hasPermission && !loading ? 'filter blur-sm pointer-events-none select-none opacity-50' : ''
          }`}
      >
        {children}
      </div>
