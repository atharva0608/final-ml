...
export default function NodeSelector() {
  ...
  const toNodeToProvName = {};
  provisionList.forEach(p => {
    if (p.prov_node_name) {
      toNodeToProvName[p.prov_node_name] = p.prov_node_name;     // direct match
      if (p.node_name && p.node_name !== p.prov_node_name) {
        toNodeToProvName[p.node_name] = p.prov_node_name;        // IP-alias fallback
      }
    }
  });
  ...
  // pod.to_node is resolved through toNodeToProvName so IP aliases map to the
  // canonical synthetic provision name used as the lookup key in Section A.
  const provNodeSourceMap = {};
  (clusterPlan?.drain_nodes ?? []).forEach(drain => {
    (drain.pods_leaving ?? []).forEach(pod => {
      if (pod.is_new_node && pod.to_node) {
        const canonKey = toNodeToProvName[pod.to_node] ?? pod.to_node;
        if (!provNodeSourceMap[canonKey]) provNodeSourceMap[canonKey] = [];
        provNodeSourceMap[canonKey].push({
          ...
          {viewMode === 'desired' && (
            <div className="flex-1 overflow-auto p-6 space-y-8">
              ...
              {planCompleteness === 'consolidation' && (() => {
                ...
                const _reasonMap = {
                  spot_target_not_set_by_orchestrator: 'Spot targets were not wired to the Planner — check orchestrator configuration.',
                  existing_od_capacity_absorbed_spot_pods: 'Existing on-demand nodes have enough capacity to absorb all workloads. Consolidation is more cost-effective than new spot provisioning.',
                  confidence_provisional_conservative_od_preferred: `Workload classification confidence is ${_smd.confidence_gate ?? 'PROVISIONAL'}. Spot migration requires CONFIRMED confidence.`,
                  disruption_safety_blocked_all_spot_pods: 'No pods passed disruption safety checks for spot migration at this time.',
                  spot_migration_not_needed: 'All pods already placed optimally — no spot migration required.',
                };
                const _reasonText = _smd.reason ? (_reasonMap[_smd.reason] || _smd.reason) : null;
                const _showConflict = _wieSpot > 0 && _smd.selected === false && _reasonText;
                return (
                  <div className="space-y-2">
                    <div className="bg-blue-50 border border-blue-200 rounded-lg px-4 py-3 flex items-center gap-2 text-xs text-blue-800">
                      <FiServer className="w-4 h-4 flex-shrink-0 text-blue-500" />
                      <span><strong>Consolidation Plan</strong> — {drainSet.size} node{drainSet.size !== 1 ? 's' : ''} will be drained and pods bin-packed onto {keepSet.size} existing node{keepSet.size !== 1 ? 's' : ''}. No new nodes need to be provisioned.</span>
                      <span className="ml-auto px-2 py-0.5 rounded text-[10px] font-bold border flex-shrink-0 bg-blue-100 text-blue-700 border-blue-200">Consolidation</span>
                    </div>
                    {_showConflict && (
                      <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-2.5 flex items-start gap-2 text-xs text-amber-800">
                        <FiAlertTriangle className="w-4 h-4 flex-shrink-0 text-amber-500 mt-0.5" />
                        <span>
                          <strong>WIE identified {_wieSpot} spot-eligible replica{_wieSpot !== 1 ? 's' : ''}</strong>, but the Planner chose consolidation instead.
                          {' '}<span className="text-amber-600">{_reasonText}</span>
                        </span>
                      </div>
                    )}
                  </div>
              ...
              {planCompleteness !== 'loading' && planCompleteness !== 'none' && (drainSet.size > 0 || keepSet.size > 0) && rawNodes.length > 0 && (
                <div className="border border-gray-200 rounded-xl overflow-hidden shadow-sm">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-3">
                    {(clusterPlan?.drain_nodes ?? []).map((dn, i) => {
                      ...
                      // Resolve pod.to_node through reverse map so provision nodes
                      // show their synthetic prov_node_name instead of raw IPs.
                      const destNodes = [...new Set(
                        podsLeaving
                          .filter(p => p.to_node)
                          .map(p => (toNodeToProvName[p.to_node] ?? p.to_node).split('.')[0])
                      )];
                      return (
