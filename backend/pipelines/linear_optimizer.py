...
class LinearPipeline:
    ...
    def execute(self, instance_id: str) -> PipelineContext:
        ...
        # =====================================================================
        # SAFETY LATCHES - Prevent Lab Mode from touching Production resources
        # =====================================================================

        # LATCH 1: Environment Type Check
        if instance.account.environment_type != "LAB":
            raise PermissionError(
                f"🚫 SAFETY VIOLATION: Cannot run Lab Mode on PROD account. "
                f"Environment: {instance.account.environment_type}. "
                f"Use Cluster Mode for production workloads."
            )

        # LATCH 2: AWS Resource Tag Validation
        # Fetch actual instance from AWS to check tags
        try:
            account = instance.account
            ec2 = get_ec2_client(account.role_arn, account.external_id, account.region)

            ec2_response = ec2.describe_instances(InstanceIds=[instance.instance_id])
            if not ec2_response['Reservations']:
                raise ValueError(f"Instance {instance.instance_id} not found in AWS")

            ec2_instance = ec2_response['Reservations'][0]['Instances'][0]
            tags = {tag['Key']: tag['Value'] for tag in ec2_instance.get('Tags', [])}

            # LATCH 2a: Auto Scaling Group Protection
            if 'aws:autoscaling:groupName' in tags:
                raise PermissionError(
                    f"🚫 SAFETY VIOLATION: Cannot run Lab Mode on Auto Scaling Group instances. "
                    f"ASG: {tags['aws:autoscaling:groupName']}. "
                    f"ASG instances must be managed at the cluster level via Cluster Mode."
                )

            # LATCH 2b: Production Tag Protection
            environment_tag = tags.get('Environment', '').lower()
            if environment_tag in ['production', 'prod']:
                raise PermissionError(
                    f"🚫 SAFETY VIOLATION: Cannot run Lab Mode on Production-tagged instances. "
                    f"Tag: Environment={tags['Environment']}. "
                    f"Remove the tag or use Cluster Mode."
                )

            # LATCH 2c: Critical Workload Protection
            criticality_tag = tags.get('Criticality', '').lower()
            if criticality_tag in ['critical', 'high']:
                raise PermissionError(
                    f"🚫 SAFETY VIOLATION: Cannot run Lab Mode on critical workloads. "
                    f"Tag: Criticality={tags['Criticality']}. "
                    f"Lab Mode is for isolated testing only."
                )

        except ClientError as e:
            self.logger.error(f"Failed to validate AWS instance tags: {e}")
            raise PermissionError(
                f"Cannot validate instance safety: {e}. "
                "Refusing to proceed without tag validation."
            )

        # LATCH 3: Shadow Mode Enforcement
        if not instance.is_shadow_mode:
            self.logger.warning(
                f"⚠️ Lab Mode is running in LIVE mode (shadow_mode=False). "
                f"Instance will be ACTUALLY replaced if recommendation is made."
            )
