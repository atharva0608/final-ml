...
| **FE-CMP::Tag::PoliciesMgr** | `frontend/src/components/settings/TagPoliciesManager.jsx` | Component | **Updated (2026-01-30)**: Policy Builder Wizard UI for Smart Auto-Tag system. **Features**: Value Type dropdown with 8 dynamic variables (User Email, Creation Date, Org Name, Env Var, etc.), Live Preview panel showing resolved values, Smart Interface split (Automation Rules vs Governance Policies tabs), Resource Scope selector, Override Behavior toggle, System Tag injection option. | `policies`, `formData`, `showPreview`, `activeTab` | `api`, `FiIcons` |
