...
| **2026-01-12** | **Backend / Cleanup** | `Feature` | Created **Resource Hygiene & Cleanup module** with multi-region AWS scanning. Implemented `cleanup_schemas.py` (ResourceType, CleanupStatus, ResourceItem, CleanupSummary, CleanupAction, CleanupActionResponse), `cleanup_service.py` (scans all enabled AWS regions for orphaned instances, volumes, snapshots, and Elastic IPs, calculates cost savings), and `cleanup_routes.py` (GET /scan, POST /action endpoints). |
| **2026-01-12** | **Backend / API** | `Update` | Registered `cleanup_router` in `api/__init__.py`. |
| **2026-01-12** | **Frontend / Cleanup** | `Feature` | Created `CleanupDashboard.jsx` component with multi-region scanning support, tabs for different resource types, **FIXED badge count bug** (each tab now shows its own count instead of active tab's count), bulk actions (authorize/terminate/delete/release), and cost savings display. |
| **2026-01-12** | **Frontend / API** | `Update` | Added `cleanupAPI` to `api.js` with scan and execute action methods. |
| **2026-01-12** | **Docs / Catalogs** | `Update` | Updated `01_frontend_catalog.md`, `02_backend_catalog.md`, and `03_frontend_backend_gap_analysis.md` to include Cleanup module. Incremented Real API count from 35+ to 37+. |
