...
| READ | 80+ | Nearly every route and service file. Key: `cluster_routes.py`, `cluster_service.py`, `ascpai_routes.py`, `auto_rebalancer.py`, `karpenter_routes.py` |
...
| READ | 25+ | `cluster_service.py` (get_cluster_nodes_detailed, list_clusters), `auto_rebalancer.py`, `ascpai_routes.py`, `worker_routes.py` (node-metrics), `karpenter_routes.py` |
...
| READ | 14 | `multi_cluster_routes.py`, `ascpai_routes.py` (status/history), `auto_rebalancer.py` (duplicate check) |
| UPDATE | — | `auto_rebalancer.py` (status→completed/failed), `ascpai_routes.py` |
...
**CRUD:** CREATE: 3 (`action_executor.py`), READ: 11 (`execution_controller.py`, `ascpai_routes.py`, `auto_rebalancer.py`), DELETE: 1
...
**CRUD:** CREATE: 5 (`cluster_service.py`, `karpenter_routes.py`, `auto_rebalancer.py`), READ: 16 (`agent_routes.py`, `karpenter_routes.py`, `auto_rebalancer.py`, `ascpai_routes.py`), DELETE: 3
...
**CRUD:** CREATE: 3 (`pricing_collector.py`, `aws_pricing_service.py`, `ascpai_worker.py`), READ: 5 (`ml_feature_service.py`, `pool_ranking_service.py`), DELETE: 1 (cleanup)
...
| `daily_cluster_stats` | 1 | 3 | — | `ascpai_worker.py` |
...
| `spot:spend_velocity:{cluster_id}` | get | ∞ | `ascpai_worker.py` | `cluster_routes.py` (cluster summary) |
...
| `spot:active_cluster_count` | get | ∞ | `control_plane_loop.py` | `ascpai_routes.py`, `pool_ranking_service.py` |
| `spot:dryrun_count:{region}` | get, incr, expire | 3600s (1h) | `pool_ranking_service.py` | `ascpai_routes.py` |
| `spot:trigger_eval:{cluster_id}` | setex | 300s (5m) | `ascpai_routes.py` | `auto_rebalancer.py` |
| `spot:cooldown:cluster:{cluster_id}` | delete | varies | `ascpai_routes.py` | `cooldown_controller.py` |
...
| `spot:model_mismatch:{cluster_id}` | delete | ∞ | `auto_rebalancer.py` | `ascpai_routes.py` |
...
| `ascpai:pool_rankings` | get | ∞ | `pool_ranking_service.py` | `dry_run_refresher.py` |
| `blacklist:pool:{pool_key}` | exists | varies | `blacklist_service.py` | `ascpai_routes.py`, `pool_ranking_service.py` |
...
| Key Pattern | Operations | TTL | Producer | Consumer |
|---|---|---|---|---|
| `ascpai:ml_degraded` | get, set, delete | 600s (10m) | `pool_ranking_service.py` | `ascpai_routes.py` |
| `ascpai:ml_fail_count` | get, incr, expire | 600s (10m) | `pool_ranking_service.py` | `ascpai_routes.py` |
...
| `price_shock:{region}` | get | ∞ | `pricing_collector.py` | `ascpai_routes.py` |
