...
| **BE-SVC::Cleanup::Main** | `backend/services/cleanup_service.py` | Service | **Real**: Resource Hygiene & Cleanup logic with multi-region AWS scanning. Scans all enabled regions for unauthorized instances, orphaned volumes, zombie snapshots, and unused Elastic IPs. Calculates cost savings and executes cleanup actions across regions. | `Account`, `Cluster`, `Instance`, `boto3` |
...
| **BE-API::Cleanup::Main** | `backend/api/cleanup_routes.py` | API | **Real**: Resource Hygiene & Cleanup. Multi-region AWS scanning for orphaned instances, volumes, snapshots, and Elastic IPs. Execute cleanup actions (authorize, terminate, delete, release). | `CleanupService`, `boto3` |
...
| **BE-SCH::Cleanup::Main** | `backend/schemas/cleanup_schemas.py` | Schema | Pydantic Schemas for Cleanup (ResourceType, CleanupStatus, ResourceItem, CleanupSummary, CleanupAction, CleanupActionResponse). | `Pydantic` |
