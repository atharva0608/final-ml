...
| **BE-SCH::Cleanup::Main** | `backend/schemas/cleanup_schemas.py` | Schema | **Real**: Pydantic models for resource hygiene. `ResourceType` enum (11 types incl. LOAD_BALANCER, NETWORK_INTERFACE, RDS_DB, S3_BUCKET, IAM_USER), `CleanupStatus` (RISK, LEGACY_UPGRADE), `ResourceItem` with `reason` field, `CleanupActionType` (SNAPSHOT_STOP, DISABLE). | `Pydantic` |
