...
> **Files analyzed**: 165+ backend Python services, 8 agent files, 70+ models, 36 workers, 20 core modules
> **Depth**: Function-level analysis with exact line numbers, retry logic, fallback paths, and internal algorithms
> **Verified**: auto_rebalancer.py (5,695L), pool_ranking_service.py (2,470L), auto_scaler.py (346L), pool_reputation_service.py (210L), emergency_rebalancer.py, discovery.py (955L), circuit_breaker.py, base.py
...
| `global_ema.decay` | crontab(hour=2, minute=0) | **NEW** Daily EMA decay of all pool interruption rates (30-day half-life decay, nightly at 2 AM UTC) |
...
**Total beat tasks: 44** (43 previously + `global_ema.decay` added 2026-03-27)
...
|---|---|---|
| `clusters` | `cluster.py` | EKS cluster metadata, status, settings (74 columns; includes `is_dismissed` Boolean from `20260327_add_cluster_dismissed_degraded`) |
| `instances` | `instance.py` | EC2 instances — 20 columns: id, cluster_id, account_id, instance_id, instance_type, lifecycle (SPOT/ON_DEMAND), az, price, cpu_util, memory_util, state, status, status_message, architecture, node_name, launched_by, standby, created_at, updated_at, last_heartbeat |
...
| `global_pool_ema` | `global_pool_ema.py` | Cross-customer EMA interruption rate per pool (PK: pool_key `instance_type:az`; columns: count, rate, peak_rate, sample_clusters, last_event) |
| `global_pool_ema_history` | `global_pool_ema.py` | Analytics-only audit trail of EMA rate snapshots (pool_key, rate, count, recorded_at) |
...
**Cluster cleanup grace periods** (`_cleanup_deleted_clusters()`, code-verified 2026-03-27, lines 624-689):
- **Grace 1 (all clusters)**: Never delete clusters created < 60 minutes ago (`minutes_since_created < 60` → skip)
- **Grace 2 (agent clusters)**: Agent-installed clusters (`agent_installed='Y'`) NOT IN AWS scan → set status `DEGRADED` (data preserved). **NOT deleted** — they are kept in DB with DEGRADED status. Previously documented as "delete after 2h heartbeat absence" — **incorrect**
- **Grace 3 (non-agent clusters)**: Any cluster with heartbeat within last **10 minutes** → skip cleanup

**Corrected: `_cleanup_deleted_clusters` behavior (code-verified 2026-03-27)**:
```python
# Line 645-653: Grace 1 — skip new clusters
if minutes_since_created < 60: continue

# Line 656-663: Grace 2 — mark agent clusters DEGRADED (not delete)
if db_cluster.agent_installed == 'Y':
    clusters_to_degrade.append(db_cluster)  # DEGRADED status, data preserved
    continue

# Line 666-671: Grace 3 — skip recent heartbeats
if minutes_since_heartbeat <= 10: continue
```
...
Current HEAD: `20260327_global_pool_ema` (verified 2026-03-27)

```
20260327_global_pool_ema                             ← HEAD (global EMA pool interruption rates table; down_revision: 20260327_cluster_dismissed)
  ↑
20260327_add_cluster_dismissed_degraded              (is_dismissed column on clusters; down_revision: 20260325_cooldown_states)
  ↑
20260325_add_max_concurrent_and_cooldown_states      (ClusterCooldownState table + max_concurrent_rebalance_actions; down_revision: 20260325_state_entered_at)
  ↑
20260325_add_state_entered_at_trigger                (PostgreSQL BEFORE UPDATE trigger for state_entered_at + backfill; down_revision: 20260323_launch_outcomes)
  ↑
20260323_add_launch_outcomes_reputation_constraints  (launch_outcomes table, state machine columns, unique constraints)
  ↑
...
20260309_cleanup_volume_tables   (previous stable HEAD)
  ↑
001_base_schema.py / 002_seed_data.py              (initial schema)
```

**Total migrations: 25 files** (last 2 added 2026-03-27)
...
**Data flow (code-verified 2026-03-27)**:
1. `MLFeatureService.engineer_features()` → 45-feature vector per pool
2. `classifier_6.onnx` → `risk_probability` (clamped `[0, 1]`)
3. `regressor_6.onnx` → `predicted_savings` (clamped `[0, 1]`)
4. **4-signal blended risk** (`compute_blended_risk`, pool_ranking_service.py:1375-1428):
   ```
   base_weight signals: ONNX risk=0.40, Price pressure=0.35, Spot Advisor=0.25
   + optional EMA historical (max 40% weight blend)
   Hard overrides: blacklist → 0.75, dry_run_fail → 0.65
   ```
5. **Spot Advisor savings override** when ONNX regressor saturates (predicted_savings ≥ 0.99):
   - Use `sa_savings / 100.0` from `spot_advisor:{region}:{type}:Linux` key
   - Fallback: `(ondemand_price - spot_price) / ondemand_price`
   - Bounds: `max(0.30, min(0.95, value))`
6. Apply pool reputation multiplier from `PoolReputationService.bulk_get_multipliers()` (see §4.5)
7. **Unified composite score** (`compute_unified_score`, pool_ranking_service.py:1352-1372):
   ```python
   score = (savings_pct * savings_weight) * (1.0 - final_risk) * reputation_mult * capacity_mult
   # savings_weight default: 0.80
   # reputation_mult: from pool_reputation_service (0.80–1.10)
   # capacity_mult: 1.05 if dry_run PASS, 0.0 if dry_run FAIL, 1.0 if no data
   ```
   - **NOT** the deprecated `savings × (1 - risk)` formula — that is kept only in `scoring.py` for legacy ranking sort order
8. Hard filter: pools where `final_risk > risk_threshold (0.35)` rejected from CACHE output
9. **Tiered Spot Advisor filter** (5 passes before step 7, stops when `global_limit=1500` met):
...
10. Store top 1500 pools per region in Redis: `global_pool_rankings:{region}` (65-min TTL = 3900s)
...
If ONNX inference fails **5+ times** in 10 minutes (verified: threshold is `>5` not `3`):
...
**Reputation multiplier tiers** (from `pool_reputation_service.py`, code-verified 2026-03-27, lines 32-40):

```python
def _compute_reputation_mult(success_rate: float, avg_uptime_hours: float) -> float:
    if success_rate >= 0.95 and avg_uptime_hours > 168:  return 1.10  # excellent
    if success_rate >= 0.85:                              return 1.05  # good
    if success_rate >= 0.70:                              return 0.95  # below-average
    return 0.80                                                        # poor
```
...
| No data yet (< 3 samples = `MIN_SAMPLE_THRESHOLD`) | 1.00 | Neutral |

Redis key: `pool_reputation:{pool_key}`, TTL 3600s. Rolling 30-day window, max 500 rows per pool.

**Note on threshold interpretation**: The decision_engine.py weighted formula (`0.5 + success_rate × 0.7`) documented in memory.md is used only for the manual computation in Section 6 docs. The canonical tiers above from `pool_reputation_service.py` are used by the ML pipeline (tiered step-function, NOT continuous).
...
### 4.6 Global Pool EMA — Cross-Customer Interruption Tracker (code-verified 2026-03-27)

**Files**: `backend/services/global_ema_service.py`, `backend/workers/tasks/global_ema_tasks.py`

**Purpose**: Aggregate, cross-customer EMA (Exponential Moving Average) of spot pool interruption rates. Provides a long-memory signal that persists across Redis restarts (dual-stored in Redis + Postgres).

**Storage**:
- **Redis (hot cache)**: `global:pool:interruption:{pool_key}` (TTL = 90 days); JSON dict with `count`, `rate`, `peak_rate`, `last_updated`, `last_event`, `sample_clusters`
- **DB (durable)**: `global_pool_ema` table (PK: `pool_key = 'instance_type:az'`)
  - Columns: `pool_key`, `instance_type`, `az`, `region`, `count`, `rate` (Numeric 5,2), `peak_rate`, `sample_clusters`, `last_event`, `created_at`, `updated_at`
  - Indexes: `idx_global_pool_ema_region`, `idx_global_pool_ema_last_event`
- **History table** (analytics): `global_pool_ema_history` — appended on each update: `pool_key, rate, count, recorded_at`

**Key formulas** (from `global_ema_service.py` docstring):
```python
# Decay (daily job at 2 AM): 30-day half-life
rate *= 0.5 ** (days_since_last_event / 30.0)

# EMA update on each new interruption event (α = 0.1, MAX_RATE = 25.0):
new_rate = old_rate * (1 - 0.10) + max_rate * 0.10

# Confidence score (approaches 1.0 as sample count grows):
confidence = min(0.95, 1.0 - 0.5 ** (count / 200.0))

# Weight contribution to risk blend in compute_blended_risk():
ema_weight = confidence * 0.40   # max 40% when count ≥ 200 clusters
```

**Constants**: `EMA_ALPHA = 0.1`, `MAX_RATE = 25.0`, `DECAY_HALF_LIFE_DAYS = 30`, `REDIS_TTL_SECONDS = 7,776,000` (90 days)

**Lifecycle**:
1. `get_or_create_ema()` — lazily initializes entry on first spot launch for a pool
2. `update_ema_on_interruption(pool_key, ...)` — called on each ITN event → updates Redis, enqueues `global_ema.persist` task for async DB upsert
3. `global_ema_decay` Celery task (nightly 2 AM) → calls `decay_all_pools(redis, db)` → decays all Redis entries + syncs to DB
4. `get_ema_stats(redis, db, pool_key)` — read-through: Redis → DB fallback → `None`

**Integration into ML risk blend**: When EMA data exists for a pool, `compute_blended_risk()` in `pool_ranking_service.py` reads the rate and confidence, then adds `ema_weight = confidence × 0.40` to the weighted blend. The base weights (ONNX=0.40, Price=0.35, SA=0.25) are renormalized to accommodate EMA's contribution (max +40%).

### 4.7 Market View — REMOVED (2026-03-27)

> **Market View has been completely removed from the frontend.** The `PoolRankings.jsx` component no longer renders a Market View tab, and all associated state variables, fetch functions, and polling logic have been deleted. The backend endpoint `GET /api/v1/ascpai/clusters/{id}/market-view` still exists but has no frontend consumer.

**Reason:** Market View duplicated information already visible in the Node-Specific View and Per-Node Alternatives. The Node-Specific View provides more actionable per-node context.

**PoolRankings.jsx tabs (current):** Node-Specific View (default) | Cluster Impact View

**Removed from frontend:**
- Market View tab button and all content (table, pagination, stats bar, legend)
- 14 state variables (`marketViewPools`, `marketViewLoading`, etc.)
- `fetchMarketViewPage()`, `handleMarketViewSort()` functions
- TTL countdown and market view polling useEffects
- Default tab changed from `'market'` to `'node'`

### Market View Backend Endpoints (still active, no frontend consumer)

POST /api/v1/ascpai/pools/rankings → (no UI consumer after market view removal)
  Fleet-wide rankings, no source node context, m5.large neutral baseline

GET /api/v1/ascpai/clusters/{id}/market-view → (no UI consumer after market view removal)
...
### 4.7 Market View Known Bugs — Root Cause & Fix (2026-03-24) (HISTORICAL)
...
Used by reconciliation worker and the per-node alternatives API endpoint (`GET /api/v1/ascpai/clusters/{cluster_id}/nodes/{node_id}/alternatives`) to find alternative pools for a specific node.

**Endpoint implementation** (`backend/api/ascpai_routes.py`, `get_node_alternatives()`):

| Step | Logic |
|---|---|
| 1 | Load all pools from `market_view_cache:{cluster_id}` or `global_pool_rankings:{region}` → `all_pools` |
| 2 | Load settings: `OptimizationStrategy` → `risk_ceiling_pct`, `tradeoff_pct`; `ClusterOptimizationSettings` → `architecture_preference`, `diversify_pools` |
| 3 | Load blacklist: Redis set `risky_pools` → `_blacklisted_keys` (set of `{instance_type}:{az}` strings) |
| 4 | Build occupied pool set (when `diversify_pools` enabled): query all running instances in cluster → `_cluster_occupied_pools` (set of `{instance_type}:{az}`) |
| 5 | **Blacklist gate**: skip pools where `{instance_type}:{az}` is in `_blacklisted_keys` |
| 6 | **Size gate**: `pool_vcpu >= node_vcpu` AND `pool_mem >= node_mem` (minimum resource floor, no upper bound) |
| 7 | **Architecture gate** (respects `architecture_preference` setting): `amd64` → only x86/amd64 pools; `arm64` → only ARM pools; `both` → hardware compatibility with source node |
| 8 | **Risk ceiling gate**: `aws_risk > risk_ceiling` → skip (uses AWS Spot Advisor data) |
| 9 | **Interruption rate hard cap**: pools with AWS interruption rate ≥ 15% are always excluded |
| 10 | Compute `savings_pct`, `node_ev`, `unified_score` per candidate |
| 11 | Split into positive (savings ≥ 0) and negative sets |
| 12 | **Negative savings gating**: if positive set is non-empty → discard negative set entirely. If positive set is empty → keep only negatives within `tradeoff_pct` (e.g., −20% max) |
| 13 | **Diversification** (when `diversify_pools` enabled): exclude pools whose `instance_type:az` is already occupied by a running cluster node + max 2 entries per instance family. Fallback: if strict mode yields < 3 results, relax to family-cap-only (without occupied-pool exclusion) |
| 14 | Paginate and return |

### 5.4.1 `node-recommendations` — Live Table Target Pool Selection

Used by the live rebalancing table (`Node | Current Type | Target Pool | Status | Cost/Hr | Cost Trend | Action | Savings`) to select the best target pool per node.

**Endpoint**: `GET /api/v1/ascpai/clusters/{cluster_id}/node-recommendations` (`ascpai_routes.py:~1302`)

| Step | Logic |
|---|---|
| 1 | Load global or cluster-level pool rankings |
| 2 | When `diversify_pools` enabled: query all running instances → build `_occupied_pools` set of `(instance_type, az)` pairs and `_occupied_types` set |
| 3 | Per node: filter pools by size (vCPU, memory floor), architecture, blacklist |
| 4 | **Pass 1 (strict)**: Exclude pools where `(instance_type, az)` already occupied OR `instance_type` already in use; enforce 15% interruption rate hard cap; sort by risk then savings |
| 5 | **Pass 2 (relaxed)**: If Pass 1 yields nothing, relax diversity checks (no occupied pool/type exclusion); still enforce 15% interruption rate hard cap |
| 6 | Return best target pool per node with savings estimate |

**Post-rebalance UI refresh**:
- After successful Step 6 in auto_rebalancer, `redis.delete(f"cluster_coverage:{cluster_id}")` invalidates the coverage cache
- Frontend 30s polling picks up fresh data: terminated source node disappears, replacement node appears
- If `selectedNodeId` points to a terminated node, dropdown resets to first available node

**`decision_engine_service.py` `rank_for_node()`** (internal pipeline for rebalancer):
...
### 8.5 Right-Sizing Recommendations (`GET /api/v1/karpenter/recommendations`)

**Source**: `backend/api/karpenter_routes.py` (lines 793-1079)

This endpoint returns per-node right-sizing and spot pool recommendations for all clusters (any `karpenter_mode`). When `cluster_id` is provided, ALL instances in the cluster are analyzed regardless of mode. Without `cluster_id`, only `DRY_RUN` clusters are returned.

#### `INSTANCE_SPECS` Dict (ap-south-1 calibrated baseline prices)
45+ instance types covering `t3/t3a/t4g`, `m5/m6i/m6g`, `c5/c6i/c6g`, `r5/r6i` families. Format: `"type": (vcpu, memory_gb, od_hourly_usd)`. Example: `"m5.xlarge": (4, 16.0, 0.192)`.

#### `_get_od_price(instance_type, region, redis_client)` (lines 58-70)
Reads live OD price from Redis (tries `od_price:{region}:{type}` then `ondemand_price:{region}:{type}`). Falls back to `INSTANCE_SPECS` hardcoded price. This ensures pricing is region-accurate when Redis has current data.

#### `_bin_pack_instance(current_type, cpu_pct, mem_pct, buffer_pct=30.0, region, redis_client)` (lines 73-121)
Finds the optimal instance size given observed utilization:
```
required_vcpu = max(0.25, (current_vcpu × cpu_pct/100) × 1.30)
required_mem  = max(0.50, (current_mem  × mem_pct/100) × 1.30)
```
- **Under-fit** (current doesn't cover required): returns cheapest **larger** type from `_priced_candidates(h > c_hourly)` that fits requirements
- **Over-fit** (current covers requirements): returns cheapest **smaller** type from `_priced_candidates(h < c_hourly)` that still fits
- `delta_monthly = (current_hourly - recommended_hourly) × 730` — positive = savings, negative = upsize cost increase
- **`_priced_candidates(filter_fn)`**: builds candidate list with live Redis OD pricing (`_get_od_price()` per type), sorted by price ascending

#### Template constraint enforcement (lines 927-951)
If the cluster has an active `NodeTemplateVersion` with `allowed_families`, the bin-packed result is validated. If the recommended type's family is not in `allowed_families`, a secondary bin-pack restricted to allowed families is performed.

#### Spot pool selection for stateless nodes (lines 954-985)
For non-spot stateless nodes, `PoolRankingService.rank_pools()` is called with a `NodeTemplate` scoped to the recommended type's vCPU/memory bounds (+ template constraints). Returns the top-1 ML-scored pool as `spot_pool` dict.

#### Response object per node (lines 1037-1062)

| Field | Source | Notes |
|---|---|---|
| `current_type` | DB `Instance.instance_type` | |
| `recommended_type` | `_bin_pack_instance()` | May equal `current_type` if already optimal |
| `is_upsize` | `resize_savings < 0` | `True` when node is over-utilised (Critical impact) |
| `spot_pool` | `PoolRankingService` top-1 | `None` for stateful or already-spot nodes |
| `ev_pct` | `min(99, savings_pct)` | |
| `impact` | `"Critical"` (upsize), `"High"` (resize_savings>10), `"Neutral"` | |
| `risk_prob` | `spot_pool.risk_score × 100` or default 15 | |
| `node_type` | WorkloadInspector Redis key `spot:node_classification:{cluster_id}` | `"stateful"` if `STATEFUL_PROTECTED`/`DRAIN_UNSAFE`, else `"stateless"` |
| `potential_savings` | Combined resize + spot migration savings | Negative if upsize |

#### Strategy logic by node classification

| Condition | Strategy |
|---|---|
| `is_spot = True` | Already spot — no changes (shows lifecycle-optimized) |
| `is_upsize = True` | Upsize alert — Critical impact, negative savings |
| `stateless` + spot pool | Resize + spot migration combined savings |
| `stateless`, no spot pool | Resize only + convert to spot (70% fallback savings) |
| `stateful` | Resize only (OD, no spot migration), always requires approval |
...
5. **Emergency rebalancing — 3 execution paths (code-verified 2026-03-27)**:
   - **Path 1 — Karpenter** (checked FIRST, lines 140-148): if `spot:karpenter:installed:{cluster_id}` Redis key exists → `_execute_karpenter_emergency()`. Creates 3 sequential `AgentAction` records all with `priority=10`: `CORDON_NODE` → `DRAIN_NODE` (grace_period=90, force=True, emergency=True) → `TERMINATE_NODE` (payload: `termination_mode="karpenter"`, reason="emergency_karpenter"). Direct EC2 terminate — zero ASG detach/suspend/resume calls. Karpenter's `consolidationPolicy: WhenUnderutilized` auto-provisions replacement node once the old node is gone. **Note**: 2-hour cluster cooldown is NOT set for this path (see step 7).
   - **Path 2 — Standby failover** (if `maintain_standby=True` and standby exists): UNCORDON standby → CORDON interrupted → DRAIN (grace_period=90, force=True) → TERMINATE → launch new standby async. Sets 2-hour cluster cooldown after completion.
   - **Path 3 — Normal emergency** (fallback): Creates `RebalancingAction` with `trigger='emergency'` and `priority=10`. Sets 2-hour cluster cooldown after completion.
...
7. **2-hour cluster cooldown set**: After standby failover (Path 2) or normal emergency (Path 3), `key_cluster_cooldown(cluster_id)` is set with a 2-hour TTL (`EMERGENCY_COOLDOWN_MINUTES × 60`). The Karpenter path (Path 1) returns before reaching the cooldown set code — no cooldown is set, relying on Karpenter's own node lifecycle management.
...
**Guard 2.5 / auto_rebalancer — P-M4 fix (pool rankings cache miss skip)**:
When `global_pool_rankings:{region}` is absent from Redis (Redis restart, cold start, TTL expiry between hourly rebuilds), the auto_rebalancer cluster loop now sets `_pm4_skip_cluster = True` and issues a `continue` instead of falling through to `PoolRankingService.rank_pools_for_size()`. Previously the cache miss path continued processing and called the full DB pipeline (JOIN on spot_prices, instance_catalog, spot_advisor) on every 15-second Celery beat — up to 240 heavy DB calls/hour per cluster during a Redis restart, exhausting the DB connection pool. A CRITICAL-level dedup log (1h key `ranking_stale_warned:{region}`) is emitted once per hour. AWS sync (`_sync_instance_state_from_aws`) still runs before the skip so instance states remain fresh even when the cache is cold.

**Per-node active action lock (changes.md §2.2 — implemented 2026-03-26)**:
Before creating a `RebalancingAction`, the rebalancer checks `spot:node_active_action:{instance_id}` (600s TTL). If the key exists, the node already has an in-flight migration (cordon/drain/replace) and is skipped to prevent overlapping drains. The key is set immediately after `db.add(rebalancing_action)` and cleared in the completion handler on both success and failure paths. This prevents the cluster-growth scenario where two 15s cycles both target the same OD node.

**Discovery sync key (changes.md §2.3 — key written, main-loop check deferred)**:
`discovery.py:scan_eks_clusters` now writes `spot:discovery_last_updated:{cluster_id}` (TTL=600s) after each cluster DB commit. The auto_rebalancer check guard (skip if key absent/old) has not been wired in the main loop — this is deferred because the current system already has sufficient staleness protection via the instance state reconciliation pass and per-cycle DB reads. The key is available for future use.

**Per-cluster interval gate — P-M6 fix (fires at default 15s)**:
The per-cluster interval gate condition was `_check_interval > 15` (strict greater-than). At the default `check_interval_seconds = 15`, the condition `15 > 15 = False` meant the `spot:last_check:{cluster_id}` Redis key was never written — the Celery backpressure burst protection was non-functional for all clusters at the default interval. Changed to `_check_interval >= 15` (inclusive): the key is now always written at any configured interval ≥ 15s. TTL is set to `max(1, _check_interval - 14)` so the key expires just before the next eligible beat, preventing double-processing during backpressure bursts.
...
| Per-node active action lock | `spot:node_active_action:{instance_id}` (600s, changes.md §2.2) | No (cleared on completion/failure) |
...
| **Emergency rebalancer path** (standby failover or normal — NOT Karpenter path) | **24 hours** via `key_blacklist_global()` — written in addition to the 15-min tiered blacklist from `report_termination()` |
...
| Per-node active action | `spot:node_active_action:{instance_id}` | 600s / 10 min (changes.md §2.2) | Yes (cleared in success + failure completion handlers) |
| Discovery last-updated | `spot:discovery_last_updated:{cluster_id}` | 600s / 10 min (changes.md §2.3) | No (written by discovery.py; read by rebalancer for staleness checks) |

---

### 10.4 Concurrency Controls (`ClusterOptimizationSettings`)

**`architecture_preference`** (`ClusterOptimizationSettings.architecture_preference`, default `'both'`)

Controls which CPU architectures are considered in Per-Node Alternative Pools:
- `both` — accepts all architectures, but enforces hardware compatibility with the source node (e.g., amd64 node → only amd64/x86_64 pools)
- `amd64` — only x86/amd64 pools regardless of source node architecture
- `arm64` — only ARM64/Graviton pools regardless of source node architecture

UI: CPU Architecture dropdown in Optimization Settings tab (ClusterList.jsx).
API: GET/PUT `/api/v1/clusters/{id}/optimization-settings` → `automation_controls.architecture_preference`.

**`max_concurrent_rebalance_actions`** (`ClusterOptimizationSettings.max_concurrent_rebalance_actions`, default `NULL` = 1)

Controls the maximum number of simultaneous in-flight `RebalancingAction` rows (status `in_progress`, `pending`, `waiting_agent`, or `pending_approval`) allowed per cluster. The gate fires in the main `for instance in on_demand_instances` loop:
...
if len(_active_rebalancing) >= _max_concurrent:
    break  # skip creating more actions this cycle
```

Setting this to `2` allows two nodes to be drained in parallel — use only for large clusters (≥10 nodes) where the drain bandwidth justifies it. Setting >1 also requires `max_concurrent_rebalance_actions` to be set in the DB record (NULL defaults to 1).

**`ClusterCooldownState` DB table** — Redis-restart resilient stabilization lock

Schema (`backend/models/cluster.py`):
```
cluster_cooldown_states
  cluster_id        VARCHAR PK (FK → clusters.id)
  stabilization_until  TIMESTAMP  — absolute UTC time when lock expires
  last_action_at    TIMESTAMP  — when the last action that set the lock occurred
  updated_at        TIMESTAMP  — auto-updated on every write
```

Written by `CooldownController.acquire_stabilization_lock()` immediately after the Redis `SETEX`. On Redis restart (TTL miss), `auto_rebalancer.py` queries this table and re-hydrates the Redis key if `stabilization_until > utcnow()`. This prevents a full-cluster burst of 15s rebalancer cycles during Redis cold-start.
...
| **Pool rankings cold** | `global_pool_rankings:{region}` absent from Redis | P-M4 fix: skip cluster entirely (`_pm4_skip_cluster = True`) — CRITICAL log emitted once per hour via dedup key | Prevents 240 heavy DB pipeline calls/hour during Redis restart |
| **Interval gate burst** | Celery backpressure queues multiple beats | P-M6 fix: `spot:last_check:{cluster_id}` key now written at default 15s interval (condition `>= 15`; TTL = `max(1, interval - 14)`) | Prevents multiple simultaneous beats from processing the same cluster |
...
- **Agent auth**: Bearer token (`API_TOKEN`) set during installation; all `agent_routes.py` endpoints enforce `validate_api_key` dependency. All `orchestrator_routes.py` endpoints also use `validate_api_key` (fixed 2026-03-24 — previously some orchestrator endpoints lacked auth).
- **Action verification**: HMAC-SHA256 with `SECRET_KEY` on every action payload. Key rotation: generate a new key and update `SECRET_KEY` in the agent DaemonSet environment. Old HMAC tokens become invalid immediately. No key expiry mechanism exists — rotation is manual.
...
- **`/metrics/*` routes**: Served WITHOUT `/api/v1` prefix — direct Nginx passthrough. No auth layer (same as a public metrics endpoint). If Nginx config changes these routes break silently. See §13 "Non-Standard Route Prefixes".
...
- **Database**: PostgreSQL with JSONB columns for flexible metadata; RDS Multi-AZ for production
  - **SQLAlchemy pool settings** (`backend/models/base.py`, code-verified): `pool_size=20`, `max_overflow=30`, `pool_recycle=1800` (30-min idle connection recycle). Max simultaneous connections = 50 (pool + overflow).
  - **`get_db()`** — FastAPI dependency (generator); yields session and closes on exit (lines 38-50)
  - **`get_db_contextmanager()`** — Python context manager for use outside FastAPI (lines 55-71)
...
## Pool Rotation Service

**File**: `backend/services/pool_rotation_service.py`
**Celery tasks**: `pool_rotation.check_all_clusters` (5 min), `pool_rotation.refresh_all_caches` (15 min)
**Worker**: `backend/workers/tasks/pool_rotation_worker.py`

Automatically rotates clusters away from degraded pools and maintains a fresh per-cluster "verified pool" cache.

### How It Works

1. **Health analysis** (every 5 min): For each active cluster, checks the current pool's health against:
   - Termination rate (ITNs in last 30 min)
   - Blacklist status (`risky_pools:{region}`)
   - Dry-run failure flag (`dry_run:{type}:{az}`)
   - Spot Advisor interruption index (>3 → elevated risk)
2. **Rotation trigger**: If health score drops below threshold, the service selects the next-best pool from `global_pool_rankings:{region}` and creates a `RebalancingAction` with trigger=`pool_rotation`.
3. **Safety gates**: Three checks before any rotation:
   - Stabilization lock active → skip
   - Recent rotation cooldown (`spot:pool_rotation:cooldown:{cluster_id}`, TTL=30min) → skip
   - Cascade threshold: if >70% of pools are failing region-wide → suspend auto-rotation for 30 min (`spot:blacklist_suspended:{region}`)
4. **Distributed lock**: `pool_rotation:lock:{cluster_id}` (Redis NX, TTL=5min) prevents parallel workers from rotating the same cluster simultaneously.
5. **Cache refresh** (every 15 min): Re-scores and caches top-N pools for each cluster into `verified_pool_set:{cluster_id}` for fast selection by the rebalancer.

### Force Rotation

`POST /api/v1/pool-rotation/clusters/{cluster_id}/force` — immediately triggers rotation bypassing the cooldown (requires operator auth). This is the correct resolution when a cluster is stuck on a degraded pool after an ITN wave.

---

## ASCP Built-in Auto-Scaler

**File**: `backend/workers/tasks/auto_scaler.py`
**Celery task**: `workers.auto_scaler.run` (30s)
**Toggle**: `ClusterOptimizationSettings.enable_ascp_auto_scaler` (default `False`)

An **optional** built-in Kubernetes cluster auto-scaler. Disabled by default — external scalers (Karpenter, Cluster Autoscaler) remain in full control unless this is explicitly enabled.

### Gates (all must pass)
1. `enable_ascp_auto_scaler = True` — per-cluster opt-in
2. `auto_rebalance_enabled = True` — rebalancer must be active
3. `cluster.karpenter_mode != AUTO` — conflict avoidance (Karpenter AUTO manages its own capacity)

### Scale-Up Logic
- Detects pending pods via `kubectl get pods --field-selector=status.phase=Pending`
- If pending pods exist and no in-flight OD→spot conversion is already running → queues a new node launch
- **Rate limit**: `cluster:scaler:cooldown:{cluster_id}` (Redis, TTL = 30s) — max 1 scale-up action per 30-second cycle
- **Target count**: `cluster:target:{cluster_id}` (Redis, TTL = 7 days) — desired running-node count; updated when pending pods are detected

### Scale-Down Logic
- If `avg(cpu_util + mem_util) < scale_down_threshold` continuously for the stabilization window → queues a `CORDON` `AgentAction` for the least-loaded node
- Graceful: does not terminate the node directly — waits for drain completion before decrementing ASG

### Not included
- Does not replace Karpenter or Cluster Autoscaler — it is an alternative for clusters where those tools are not available
- No UI for configuring thresholds; settings are in `ClusterOptimizationSettings` DB columns

---

## Cache Warmer & Dry-Run Refresher

### Cache Warmer (`cache_warmer.py`)
**Celery task**: `cache_warmer` (hourly)

Pre-computes ML pool rankings for common instance profiles (standard vCPU/memory combinations across all active regions) and writes them to `global_pool_rankings:{region}`. This ensures the rebalancer always finds a warm cache even before the first user request or cluster registration. Without the cache warmer, the first rebalancer cycle after deployment would miss the Redis cache and either skip all clusters (P-M4 guard) or fall through to the heavy DB pipeline.

### Dry-Run Refresher (`dry_run_refresher.py`)
**Celery task**: `dry_run_refresher` (5 min)

Proactively validates spot capacity for the top 100 pools (by ML score) in each active region via `ec2.run_instances(DryRun=True)`. Writes results to `dry_run:{instance_type}:{az}` with TTL 480s. This means the rebalancer's dry-run cache is always at most 5 minutes stale, and the 8-minute TTL ensures a previous "PASS" isn't evicted before the next refresh cycle.

Key interaction: when `InsufficientInstanceCapacity` fires at actual launch time, `auto_rebalancer.py` immediately invalidates the cache for that pool (`invalidate_dry_run_cache(type, az, redis, mark_failed=True)`) so the pre-check reflects real capacity without waiting for the refresher.

---

## Troubleshooting FAQ

### Node stuck provisioning (spot instance launched but never joins K8s)

**Symptom**: `RebalancingAction` status stays `waiting_agent`, node visible in EC2 but not in `kubectl get nodes`.

**Root cause**: kubelet fails to start (common: missing user-data, wrong AMI, VPC security group missing port 10250).

**Resolution steps**:
1. Check EC2 console → instance system log for kubelet errors.
2. Verify user-data: `aws ec2 describe-instance-attribute --instance-id i-xxx --attribute userData`.
3. If `spot:node_joined:{instance_id}` Redis key is absent after `spot_join_timeout` (default 300s), the rebalancer sets `spot:launch_blocked:{cluster_id}:{type}:{az}` (5-min blacklist) and rolls back.
4. To unblock immediately: `redis-cli DEL spot:launch_blocked:{cluster_id}:{type}:{az}`.

---

### Agent not connecting / action stuck in PENDING for >15 min

**Symptom**: AgentActions remain `PENDING` indefinitely; rebalancer skips all cycles because `active_agent_actions > 0`.

**Root cause**: Agent DaemonSet not running, network policy blocking WebSocket, or invalid `API_TOKEN`.

**Resolution steps**:
1. `kubectl get pods -n spot-optimizer` → check agent pod status.
2. Check agent logs: `kubectl logs -n spot-optimizer <pod>` for auth errors.
3. Verify `API_TOKEN` matches `clusters.api_key` in DB.
4. The rebalancer auto-expires `PENDING/PICKED_UP` actions older than 15 min (Celery cycle). Wait one cycle or manually: `UPDATE agent_actions SET status='EXPIRED' WHERE status IN ('PENDING','PICKED_UP') AND created_at < NOW() - INTERVAL '15 minutes';`

---

### Orphan EC2 instance (spot launched, old OD still running)

**Symptom**: `kubectl get nodes` shows extra node; AWS console shows both old OD and new spot running; cluster cost doubled.

**Root cause**: EC2 `terminate_instances` failed after drain; `spot:term_failed:{instance_id}` key set.

**Resolution steps**:
1. `redis-cli GET spot:term_failed:{instance_id}` — if set, auto-retry is blocked for 4h.
2. Manually terminate the OD instance: `aws ec2 terminate-instances --instance-ids i-xxx`.
3. Delete the Redis key to allow rebalancer to re-attempt: `redis-cli DEL spot:term_failed:{instance_id}`.
4. If the instance is in an ASG, also check desired capacity: `aws autoscaling describe-auto-scaling-groups`.

---

### Circuit breaker stuck in HALT

**Symptom**: Cluster rebalancing completely stopped; `GET /api/v1/admin/circuit-breakers` shows `state=HALT`.

**Root cause**: ≥3 rollbacks detected within 1-hour window.

**Resolution steps**:
1. Investigate root cause of rollbacks via audit log: `GET /api/v1/audit?cluster_id={id}&limit=20`.
2. Check `cb:rollbacks:{cluster_id}` counter in Redis: `redis-cli GET cb:rollbacks:{cluster_id}`.
3. Auto-recovery: HALT → CONSERVATIVE after 30 min with no new failures; CONSERVATIVE → NORMAL after 2h stable.
4. Force reset (admin only): `POST /api/v1/admin/circuit-breakers/{cluster_id}/reset`.

---

### Pool rankings not loading (HISTORICAL — Market View removed 2026-03-27)

> **Note:** Market View tab has been removed from the frontend. This section is retained for backend debugging only.

**Symptom**: Per-node alternatives show no results; backend `global_pool_rankings:{region}` empty.

**Root cause**: `global_pool_rankings:{region}` Redis key absent (Redis restart or hourly `build_global_pool_cache` task failed).

**Resolution steps**:
1. `redis-cli EXISTS global_pool_rankings:ap-south-1` — if 0, trigger rebuild: `celery call build_global_pool_cache --args '["ap-south-1"]'`.
2. Check `backend.workers.tasks.cache_builder` in Celery worker logs for errors.
3. Verify `workers.pricing.ingest_spot_prices` ran recently (`redis-cli KEYS spot_price:*` should have data).
4. If no spot price data: check celery worker is consuming the `pricing` queue (`-Q celery,pricing` flag).

---

### ClusterBaseline missing for legacy cluster (savings showing 0)

**Symptom**: `GET /api/v1/clusters/{id}/savings` returns `$0` for an active cluster with real spot instances.

**Root cause**: Legacy clusters created before `ClusterBaseline` table was added have no baseline row. `SavingsCalculator` requires `ClusterBaseline.baseline_monthly_cost` as the OD cost anchor.

**Resolution**: Insert a baseline manually via SQL (requires DB admin access):
```sql
INSERT INTO cluster_baselines (cluster_id, baseline_monthly_cost, baseline_node_count, created_at)
VALUES ('{cluster_id}', {estimated_od_monthly_cost}, {current_node_count}, NOW());
```
Or call the recalculate endpoint once OD price data is available.

---

### Redis restart — rebalancer burst / stabilization lock lost

**Symptom**: After Redis restart, all clusters start rebalancing simultaneously (N×15s cycles all execute in the first minute).

**Root cause**: Stabilization locks (`spot:stabilization_lock:*`) and cooldowns are lost on Redis restart.

**Resolution**: The `cluster_cooldown_states` DB table provides a persistence layer. The rebalancer re-hydrates stabilization locks from this table on cache miss (Issue 13). No manual action required — the burst typically resolves within 1-2 minutes as new locks are set after the first cycle.

---

### Session 2 — 2026-03-26 Fix Log (41 bugs fixed)

#### Decision Engine Fixes (P-C11, P-C12, P-C13, P-M29)

Three compounding bugs made the decision engine completely non-functional:

**P-C11 — PoolRankingService arg swap**: `PoolRankingService(self.redis, self.db)` — args reversed. Constructor signature is `(db: Session, redis: Redis)`. Fixed: `PoolRankingService(self.db, self.redis)`.

**P-C12 — Non-existent method call**: `ranking_svc.refresh_global_rankings(region)` — method does not exist. Fixed: `ranking_svc.build_global_pool_cache(region)`.

**P-C13 — Wrong Redis key**: Read `spot:global_rankings:{region}` (never written); writer uses `global_pool_rankings:{region}`. Fixed both read locations.

**P-M29 — Volatility bytes comparison**: `_check_volatility_regime()` compared `is_volatile == b"true"` (bytes) but `decode_responses=True` returns strings → all nodes treated as non-volatile. Fixed: `is_volatile in (b"true", "true", b"active", "active")`.

#### Right-Sizing Service Fixes (P-C6, P-C7, P-H6, P-H7, P-M8)

**P-C6 — Missing ORDER BY**: `_get_pod_metrics()` had no `order_by(PodMetric.timestamp)` → `metrics[-1]` was a random row, not the most recent sample. Fixed.

**P-C7 — Bytes volatility comparison**: Same `b"true"` vs `"true"` issue in `rightsizing_service.py`. Fixed with same multi-form check.

**P-H6 — Percentile off-by-one**: `int((p/100) * len)` returns out-of-bounds index at P100. Fixed: `int((p/100) * (len - 1))`.

**P-H7 — Null request crash**: `current_cpu_request_millicores` could be NULL in DB → `TypeError` in arithmetic. Fixed with `or None` guards.

**P-M8 — Memory truncation**: `int(bytes / 1048576)` truncates toward zero. Fixed: `math.ceil(...)`.

#### Emergency Rebalancer Fixes (P-C16, P-H16)

**P-C16 — Action stuck in `pending` forever**: `_execute_normal_emergency()` set `action.status = "pending"` — only `in_progress` is consumed. Fixed: `status = "in_progress"`.

**P-H16 — Wrong column names**: `action.target_instance_type` / `action.target_az` — those columns don't exist. Fixed: `action.actual_instance_type` / `action.actual_az`.

#### Pool Ranking Service Fixes (P-C14, P-M22, P-M25)

**P-C14 — None return on exception**: `_get_spot_advisor_data()` returned `None` implicitly on DB error; callers unpacked it as dict → `TypeError`. Fixed: explicit `return {}` in except block.

**P-M22 — Failure count inconsistency**: `_step4_blacklist_check` defaulted `failure_count = int(...) or 1`; `rank_for_node` used `or 0` → inconsistent penalty scoring. Fixed: `or 0`.

**P-M25 — vCPU range undersized**: `vcpu_min = max(1, vcpu - 1)` allowed pools with 1 fewer vCPU. Fixed: `vcpu_min = max(1, vcpu)`.

#### Cross-Account Authentication Fixes (P-C15, P-H11, P-H18)

**P-C15 — substitute_manager wrong creds + no ImageId**: Used platform creds (cross-account blind); no `ImageId` param in RunInstances → `InvalidParameterValue`. Fixed: assumed-role creds + `ImageId` from `dry_run:ami:{region}` Redis cache.

**P-H11 — tag_management_service missing ExternalId**: `sts.assume_role()` omitted `ExternalId`. Fixed: added when `account.external_id` is set.

**P-H18 — recovery_monitor missing ExternalId**: 3 separate assume_role calls omitted `ExternalId`. Fixed in all 3 locations.

#### Auto-Rebalancer Fixes (P-H5, P-H17)

**P-H5 — Diversification guard empty AZ bypass**: Empty `target_az` made occupancy check always pass → all candidates passed guard. Fixed: skip check when AZ is empty string.

**P-H17 — ASG resume passes string not Cluster**: Exception handler called `_gac_err(_cl_err.aws_role_arn, ...)` — passed ARN string instead of Cluster object. Fixed: `_gac_err(_cl_err, db)`.

#### Redis / DB Safety Fixes (P-H14, P-H28, P-H27, P-M11, P-M28)

**P-H14 — INCR+EXPIRE race** (`optimizer_coordinator.py`): Two-command incr+expire → no-TTL counter if killed between calls. Fixed: Redis pipeline.

**P-H28 — Unguarded ARN split** (5 files): `role_arn.split(':')[4]` crashes on `None` or malformed ARN. Fixed: bounds check `len() > 4`.

**P-H27 — Scraper closes caller's session**: `spot_advisor_scraper.py` closed the `db` session in `finally` even when caller owned it. Fixed: `_db_created = db is None` flag.

**P-M11 — No FK on RebalancingAction.cluster_id**: Allowed orphan rows after cluster delete. Fixed: `ForeignKey("clusters.id", ondelete="CASCADE")`.

**P-M28 — CooldownController session leak**: `_persist_cooldown()` only closed DB session on success path. Fixed: `_db.close()` moved to `finally`.

#### Karpenter Routes Fix (P-M24)

**P-M24 — 720 vs 730 hours/month**: All monthly cost multipliers corrected from `720` to `730`.

#### Frontend Fixes (P-C17, P-C18, P-C19, P-H19–P-H26, P-M17–P-M21)

**P-C17/C18** (`RightSizingDashboard.jsx`): Fake safety panel metrics and fake exposure donut charts replaced with "—"/placeholder text.

**P-C19** (`agents.py`): Auto-discovered account `000000000000` placeholder ARN replaced with `role_arn=None, is_validated="N"`.

**P-H19** (`RightSizingDashboard.jsx`): Guard added — returns early if `node.id` is a synthetic frontend ID (starts with `"stateful-"`).

**P-H20** (`RightSizingKarpenterTab.jsx`): `{clusters.length || 128}` → `{clusters.length}`.

**P-H21/H22** (`RightSizingKarpenterTab.jsx`): Hardcoded 94% efficiency and fake SVG/CSS charts replaced with "—" and "Data pending" placeholders.

**P-H23** (`PoolRankings.jsx`): TTL countdown effect dep `[ttlCounters]` → `[Object.keys(ttlCounters).length]` (prevents interval thrash on new object reference).

**P-H24** (`PoolRankings.jsx`): `marketViewPools` removed from polling effect dep array (was resetting timer on every fetch).

**P-H25** (`OverviewTab.jsx`): `price` defaults to `0`; `d.hourly || 0` guard prevents NaN in multiply.

**P-H26** (`useASCPStore.js`, `api.js`): Default `region = 'ap-south-1'` → `null` in store and API module.

**P-M17/M18** (`DecisionEngineV3Dashboard.jsx`, `RightSizingDashboard.jsx`): Retry buttons fixed with `retryCount` state pattern — increment triggers useEffect re-run.

**P-M19/M20** (`RightSizingDashboard.jsx`): Fabricated candidate sizes (`.replace()` hack) and hardcoded timestamps replaced with honest "pending" messages.

**P-M21** (`RightSizingDashboard.jsx`): Hardcoded "50%" → reads `karpenterConfig?.stateful_max_downscale_pct`, fallback `'25%'`.

**P-M26** (`api.js`): Duplicate `assignRole` definition removed.
...
| Term | Definition |
|---|---|
| **EV** | Expected Value — weighted score combining ML risk probability, predicted savings, and strategic weights |
| **S2S** | Spot-to-Spot migration — moving a running spot node from one pool to a better pool (lower risk or better diversity) |
| **RC3** | Lifecycle correction circuit — requires 3 consecutive OD lifecycle reports (~45s) before downgrading a spot node to ON_DEMAND in DB |
| **ITN** | Instance Termination Notice — AWS 2-minute warning before spot reclamation |
| **OD** | On-Demand instance — regular full-price EC2 instance |
| **IIC** | InsufficientInstanceCapacity — AWS error when spot capacity unavailable for the requested type/AZ |
| **ASCP** | Atharva Spot Control Plane — the overall platform name |
| **P-M4** | Problem category marker — P = problem, M = medium severity, 4 = issue number |
| **Guard 2.5** | The stale-pool-rankings skip gate in the auto_rebalancer cluster loop (see §4.5) |
| **Karpenter AUTO** | `KarpenterMode.AUTO` — Karpenter manages full node lifecycle; ASCP only syncs NodePool weights |
| **Karpenter PATCH** | `KarpenterMode.PATCH` — ASCP updates NodePool priorities but does not directly provision nodes |
| **SSE** | Server-Sent Events — streaming format used by agent for real-time action status updates |
| **DaemonSet agent** | The `spot-optimizer-agent` Kubernetes DaemonSet that executes cordon/drain/label operations |
| **Orchestrator** | A stateless HTTP server mode of the agent (no DaemonSet) for environments where in-cluster execution is not possible |
| **HMAC** | Hash-based Message Authentication Code — used to sign agent action payloads (`SECRET_KEY`) |
| **Pool** | A (instance_type, availability_zone) pair; the atomic unit of spot capacity management |
| **Pool Rotation** | Automatic or manual migration of a cluster from its current pool to a healthier pool |
| **Stabilization lock** | A Redis key that blocks new rebalancing cycles for 60s after any action, giving the cluster time to settle |
| **Circuit breaker** | Per-cluster state machine (NORMAL → CONSERVATIVE → HALT) that reduces or stops automation when rollbacks accumulate |
| **Warm spare** | A pre-provisioned standby spot node kept READY for zero-downtime failover during spot interruptions |
| **ClusterBaseline** | DB record anchoring the "what would OD cost?" baseline for savings calculations |
| **Bin-packing** | Selecting a smaller instance type based on actual CPU/memory utilization + 30% buffer during rightsizing |
| **WorkloadInspector** | Service that classifies K8s nodes as STATELESS_ELIGIBLE, STATEFUL_PROTECTED, SYSTEM_PROTECTED, or DRAIN_UNSAFE |
| **STATEFUL_PROTECTED** | Node classification: has StatefulSet pods, EBS PVCs, or hostPath volumes — never auto-drained |
| **DRAIN_UNSAFE** | Node classification: has PDB with maxUnavailable=0 — safe to cordon but evictions may block indefinitely |
| **TTL** | Time-To-Live — Redis key expiry duration |
| **Beat schedule** | Celery periodic task schedule (configured in `app.py`) |
| **Debounce key** | A short-TTL Redis key used to prevent duplicate actions (e.g., `ranking_refresh_pending:{region}`) |
