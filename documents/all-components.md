...
|---|---|---|---|---|---|---|---|---|---|
| **NodeList** | Table | Standalone node list with Instance ID, Type, Lifecycle badge, CPU bar, Zone — falls back to mock data on API failure | Real API (with Hardcoded fallback) | `GET /api/v1/clusters/{id}/nodes` | ClusterService → queries Instance table by cluster_id | instances | instances.instance_id, instances.instance_type, instances.lifecycle, instances.cpu_util, instances.az | clusters/ClusterDetails.jsx, shared/Card.jsx, shared/Badge.jsx | clusters/NodeList.jsx |
...
| **StrategySelector** | Card Grid | 3 strategy cards (Namespace Sleep, Nuclear, Snapshot & Restore) with icon, color, wake time, savings %, risk level, description, selected state | Real API (with Hardcoded fallback) | `GET /api/v1/hibernation/strategies` | Returns 3 strategy metadata objects | — | — | hibernation/HibernationDashboardNew.jsx | hibernation/StrategySelector.jsx |
...
| **EmergencyControls** | Card | Emergency sleep/wake buttons for all clusters or per-cluster, confirmation modals, force wake button (red), manual override with reason input | 🔴 Mock API (Hardcoded) | `POST /api/v1/hibernation/emergency/sleep` (Commented out) | API calls commented out; uses hardcoded mock arrays for schedules and clusters | hibernation_schedules, audit_logs | hibernation_schedules.cluster_ids, audit_logs.event | hibernation/HibernationDashboardNew.jsx, api/hibernation_routes.py, services/hibernation_service.py | hibernation/EmergencyControls.jsx |
| **NotificationSettings** | Card | Notification preferences: Email/Slack/Webhook toggles, threshold alerts, execution failure alerts, pre-warm notifications | 🟡 Local State Only | `GET /api/v1/hibernation/notifications/settings` (Commented out) | API calls commented out; handles state locally and uses mock timeouts for "saving" | hibernation_settings | hibernation_settings.notification_channels, hibernation_settings.alert_thresholds | hibernation/HibernationDashboardNew.jsx, api/hibernation_routes.py, services/hibernation_service.py | hibernation/NotificationSettings.jsx |
| **CostAnalyticsDashboard** | Dashboard | Hibernation cost analytics showing total saved, potential savings, ROI, and breakdowns by schedule/cluster | 🔴 100% Mock Data | `GET /api/v1/hibernation/analytics` (Commented out) | API call commented out; uses hardcoded mock `mockAnalytics` object | — | — | hibernation/HibernationDashboardNew.jsx | hibernation/CostAnalyticsDashboard.jsx |
| **CostAnalytics** | Component | Secondary cost analytics chart showing projected vs actual savings over 4 weeks with efficiency score | 🔴 Mock Data | — | Hardcoded `data` array for chart visualization and hardcoded metrics below chart | — | — | hibernation/HibernationDashboardNew.jsx | hibernation/CostAnalytics.jsx |
| **TimeBasedRules** | Component | UI for creating custom time-based waking/sleeping rules outside the standard weekly grid | 🔴 Hardcoded State | — | Initialized with hardcoded rule list in `useState`; no backend connection | — | — | hibernation/HibernationDashboardNew.jsx | hibernation/TimeBasedRules.jsx |
